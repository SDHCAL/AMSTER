...
template <typename E, typename V>
void
exampleinput(int numberhits, E trackenergy, Graph<E,V> &graph)
{
...
template <typename E, typename V>
E
examplenumberparents(Graph<E,V> graph, V parentnumber)
{
   E numberparents = 0.0;
   VIter<E,V> viter, viterend;
   for (boost::tie(viter, viterend) = boost::vertices(graph); viter != viterend; ++viter)
...
template <typename E, typename V>
E
examplenumberparentscomponent(Graph<E,V> graph, V parentnumber, V componentnumber)
{
...
{
   E efficiency = examplenumberparentscomponent<E,V>(graph,0,0)/examplenumberparents<E,V>(graph,0);
   return efficiency;
}
...
{
   E purity = examplenumberparentscomponent<E,V>(graph,0,0)/numberhitscomponent<E,V>(graph,0);
   return purity;
}

template <typename E, typename V>
void
examplecluster(Graph<E,V> &graph, std::vector<std::pair<EDesc<E,V>,E>> &p, E deltaE)
{
   int i = 0;
